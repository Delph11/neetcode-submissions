...
class Solution:
    def hasCycle(self, head: Optional[ListNode]) -> bool:
        ...
        # 4. So with while fast and fast.next: Python checks fast first If fast is None, it's falsy → stops immediately, never touches fast.next Safe
        # 5. With while fast.next and fast: Python checks fast.next first If fast is None, it tries to access None.next before even getting to check fast → crash
